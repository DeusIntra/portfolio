...
function tHeader(s: keyof I18nMessages["home"]["header"]): string {
  return t(`home.header.${s}`);
}
  ...
  <header :class="['header', { 'header--scrolled': scrolled, 'header--mobile': isMobile }]">
    <div class="container header__content">
      ...
      <nav class="header__nav" :class="{ 'header__nav--open': mobileMenuOpen }">
        <router-link to="/" class="header__nav-link" @click="closeMobileMenu">{{
          tHeader("home")
        }}</router-link>
        <router-link to="/about" class="header__nav-link" @click="closeMobileMenu">{{
          tHeader("about")
        }}</router-link>
      </nav>
